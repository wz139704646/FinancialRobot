...
class BankStatementDao:
    ...
    def add(self, voucher, bankName, companyName,clearForm,amount,date,status,balance,sumBalance):
        conn = MyHelper()
        row = conn.executeUpdate("insert into BankStatement (voucher, bankName, companyName,clearForm,amount,date,status,balance,sumBalance) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)" ,[voucher, bankName, companyName,clearForm,amount,date,status,balance,sumBalance])
        return row